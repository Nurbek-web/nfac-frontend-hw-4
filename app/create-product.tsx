  ...
  const uploadFile = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    axiosInstance
      ...
      .then((response) => {
        ...
        setFiles((prevFiles) =>
          prevFiles
            ? [...prevFiles, response.data.location]
            ...
            <div className="grid grid-cols-3 md:grid-cols-4 gap-4 mt-2">
              {files &&
                files.map((file, index) => (
                  <ImageShow
                    src={file}
                    delete={() => handleDeletePhoto(index)}
                    key={index}
                  />
                ))}
              <div className="relative group aspect-square bg-gray-100 rounded-lg overflow-hidden">
                ...
                <label
                  htmlFor="file-upload"
                  className="whitespace-nowrap rounded-md text-sm font-medium ring
                                offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:bg-accent h-10 w-10 absolute inset-0 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-50 cursor-pointer"
                >
                  <svg
            ...
            {uploadProgress > 0 && (
              <div className="absolute inset-0 bg-gray-900 bg-opacity-75 flex flex-col items-center justify-center p-4 rounded-lg shadow-lg">
                ...
                <p className="text-white mt-2 text-sm">
                  {loadedBytes.toLocaleString()} bytes of{" "}
